...
Scoop installs packages from "buckets" (repositories of package definitions). You can search for packages at [scoop.sh](https://scoop.sh) or by running:
...
Scoop organizes packages into buckets. The default bucket covers common tools, but many packages live in extra buckets like `extras`, `games`, or `nerd-fonts`. You can add buckets manually before running WinHome:
...
- Run `scoop checkup` to diagnose common configuration problems and follow the suggestions it provides.
